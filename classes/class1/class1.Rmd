...
Creating false data - also known as "sketching" 
...
sd, mean and number of data points 
...
- Yes they are different: both are higher than specified. Maybe the sample size is too small? 
...
```{r}
### Ex. 1.2a) w stepwise-instruction

# 1) defining the regression model parameters
intercept <- 30.0
slope <- 10.0
residual_sd <- 3.9 #standard deviation

# 2) Generating X values in the specified range: 0-4%
x_values <- seq(from=0, to=4)

# 3) Calculate y values based on the given regression and your newly created x-values without the error term, which gives you the y get the perfect fit line
y_values <- intercept + slope * x_values
print(y_values)

# 4) Adding random error to the line we fit above: introducing variability
error <- rnorm(length(x_values), mean = 0, sd = residual_sd)
simulated_y_with_error <- y_values + error

# 5) Prepare data for plotting: get the x-values and y-values in a dataframe to prepare for plotting w. ggplot
xandy <- data.frame(x=x_values,y=simulated_y_with_error)

# 6) Now, plot to check it out
plot(x_values, y_values, type = "l", col = "blue", lwd = 2,
     xlab = "X", ylab = "Y", main = "Hypothetical Data Sketch with Error")

points(x_values, simulated_y_with_error, col = "red", pch = 19)

legend("topleft", legend = c("Regression Line", "Simulated Data with Error"),
       col = c("blue", "red"), lty = c(1, NA), pch = c(NA, 19), lwd = c(2, NA))
```
